...
from utils.util import log_txt_as_img, exists, default, ismap, isimage, mean_flat, instantiate_from_config
from ldm_autoencoder.modules.ema import LitEma
from ldm_autoencoder.modules.distributions.distributions import normal_kl, DiagonalGaussianDistribution
from ldm_autoencoder.models.autoencoder import IdentityFirstStage, AutoencoderKL
from ldm_autoencoder.modules.diffusionmodules.util import make_beta_schedule, extract_into_tensor, noise_like
...
class DDPM(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        if self.use_scheduler:
            ...
            self.log('lr_abs', lr, sync_dist=True,
                     prog_bar=True, logger=True, on_step=True, on_epoch=False)
        return loss
    ...
    def validation_step(self, batch, batch_idx):
        ...
        self.log_dict(loss_dict_no_ema, sync_dist=True,
                      prog_bar=False, logger=True, on_step=False, on_epoch=True)
        self.log_dict(loss_dict_ema, sync_dist=True,
                      prog_bar=False, logger=True, on_step=False, on_epoch=True)
    ...
    @torch.no_grad()
    def log_images(self, batch, N=16, n_row=2, sample=True, return_keys=None, **kwargs):
        log = dict()
        x = self.get_input(batch, self.first_stage_key)
        N = min(x.shape[0], N)
...
class LatentDiffusion(DDPM):
    ...
    @torch.no_grad()
    def get_input(self, batch, k, repeat_c_by_T=False, bs=None, return_first_stage_outputs=False, force_c_encode=False,
