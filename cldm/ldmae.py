...
class AEUnetModel(nn.Module):
    ...
    def __init__(
        ...
        self.input_blocks = nn.ModuleList([
            IndexTimeEmbedSequential(conv_nd(dims, in_channels, model_channels, 3, padding=1))
        ])
        ...
        for level, mult in enumerate(channel_mult):
            for nr in range(self.num_res_blocks[level]):
                layers = [
                    ...
                    pass_module(AdaLN(index_content_embed_dim,
                                           mult * model_channels, adaln_chunk))
                ]
                ...
                if ds in attention_resolutions:
                    ...
                    if not exists(num_attention_blocks) or nr < num_attention_blocks[level]:
                        ...
                        layers.append(
                            pass_module(AdaLN(index_content_embed_dim, ch, adaln_chunk))
                        )
                self.input_blocks.append(IndexTimeEmbedSequential(*layers))
                self._feature_size += ch
                input_block_chans.append(ch)
            if level != len(channel_mult) - 1:
                out_ch = ch
                self.input_blocks.append(
                    IndexTimeEmbedSequential(
                        ResBlock(
                            ch,
                            time_embed_dim,
                            dropout,
        ...
        if legacy:
            #num_heads = 1
            dim_head = ch // num_heads if use_spatial_transformer else num_head_channels
        self.middle_block = IndexTimeEmbedSequential(
            ResBlock(
                ch,
                time_embed_dim,
                dropout,
        ...
        for level, mult in list(enumerate(channel_mult))[::-1]:
            for i in range(self.num_res_blocks[level] + 1):
                ...
                layers = [
                    ...
                    pass_module(AdaLN(index_content_embed_dim, model_channels * mult, adaln_chunk))
                ]
                ...
                if ds in attention_resolutions:
                    ...
                    if not exists(num_attention_blocks) or i < num_attention_blocks[level]:
                        ...
                        layers.append(pass_module(AdaLN(index_content_embed_dim, ch, adaln_chunk)))
                if level and i == self.num_res_blocks[level]:
                    ...
                    ds //= 2
                self.output_blocks.append(IndexTimeEmbedSequential(*layers))
                self._feature_size += ch
    ...
    def forward(self, x, index_embed, timesteps=None, context=None, **kwargs):
        ...
        for i, module in enumerate(self.input_blocks):
            h = module(h, emb, index_embed, context)
            hs.append(h)
        h = self.middle_block(h, emb, index_embed, context)
...
class AutoEncLDM(LatentDiffusion):
    ...
    def configure_optimizers(self):
        ...
        for n, p in self.model.diffusion_model.named_parameters():
            if "adaLN" in n:
                params.append(p)
            elif (n[:5] == "input" or n[:4] == "time") and \
                    not self.sd_lock_input:
                params.append(p)
            elif n[:6] == "middle" and not self.sd_lock_middle:
                params.append(p)
            elif n[:3] == "out" and not self.sd_lock_output:
                params.append(p)
