...
from ldm.modules.diffusionmodules.openaimodel import TimestepEmbedSequential, ResBlock, \
    Downsample, AttentionBlock, normalization, TimestepBlock, IndexEmbedSequential, Upsample
...
class AdaLN_ZERO(TimestepBlock):
    def __init__(self, inch, outch, multiple, dims=2):
        super(AdaLN_ZERO, self).__init__()
        assert multiple in [2, 3]
        self.chunk = multiple
        self.norm = normalization(outch)
        self.adaLN_modulation = nn.Sequential(
            nn.Linear(inch, inch),
            nn.SiLU(),
            nn.Linear(inch, outch * multiple, bias=True)
        )
...
class ControlledUnetModel(nn.Module):
    ...
    def __init__(
        ...
        self.input_adalns = nn.ModuleList([
            IndexEmbedSequential(nn.Identity())
        ])
        ...
        for level, mult in enumerate(channel_mult):
            for nr in range(self.num_res_blocks[level]):
                ...
                self.input_adalns.append(IndexEmbedSequential(*[
                    AdaLN_ZERO(inch=time_embed_dim, outch=ch, multiple=2)
                ]))
            ...
            if level != len(channel_mult) - 1:
                ...
                self.input_adalns.append(IndexEmbedSequential(nn.Identity()))
                ch = out_ch
                input_block_chans.append(ch)
                ds *= 2
                self._feature_size += ch
    ...
    def forward(self, x, index_embed, timesteps=None, context=None, control=None, only_mid_control=False, **kwargs):
        hs = []
        ...
        for i, module in enumerate(self.input_blocks):
            ...
            h = self.input_adalns[i](h, index_embed)
            hs.append(h)
...
class PreframeProcessNet(nn.Module):
    def __init__(
        self,
        image_size,
        in_channels,
        ...
        self.input_prex_block = TimestepEmbedSequential(
                    conv_nd(dims, prex_channels, 16, 3, padding=1),
                    nn.SiLU(),
    ...
    def forward(self, x, prex, timesteps, context, **kwargs):
        t_emb = timestep_embedding(timesteps, self.model_channels, repeat_only=False)
        emb = self.time_embed(t_emb)

        guided_prex = self.input_prex_block(prex, emb, context)

        outs = []

        h = x.type(self.dtype)
        for module, zero_conv in zip(self.input_blocks, self.zero_convs):
            if guided_prex is not None:
                h = module(h, emb, context)
                h += guided_prex
                guided_prex = None
            else:
...
class ControlAutoEncLDM(LatentDiffusion):

    def __init__(self,
                 preframe_config, preframe_key,
                 videoenc_config, fullvideo_key,
                 frame_index_key, index_embed_dim,
                 sd_lock_output, sd_lock_input, sd_lock_middle,
                 only_mid_control, noisy_prex, optimizer="adam",
                 *args, **kwargs):
        ...
        # process previous video frames
        self.noisy_prex = noisy_prex
        self.control_key = preframe_key
        self.control_model = instantiate_from_config(preframe_config)
        # obtain video content
        self.videocontent_key = fullvideo_key
    ...
    def p_losses(self, x_start, cond, t, noise=None):
        ...
        if self.noisy_prex:
            prex = cond['c_concat'][0]
            prex_noisy = self.q_sample(x_start=prex, t=t, noise=noise)
            cond['c_concat'] = [prex_noisy]
    ...
    def get_input(self, batch, k, bs=None, *args, **kwargs):
        x, c = super().get_input(batch, self.first_stage_key, *args, **kwargs) # x: [b c h w], c: [b c]
        # encode prex
        prex = batch[self.control_key].to(self.device)
        prex = prex.to(memory_format=torch.contiguous_format).float()
        prex = self.encode_first_stage(prex)
        prex = self.get_first_stage_encoding(prex).detach() # prex: [b c h w]
        ...
        vidcontent = rearrange(vidcontent, '(b t) c h w -> b c t h w', b=prex.shape[0])
        ...
        return x, dict(c_crossattn=[c], c_concat=[prex], c_norm=[c_norm])

    def apply_model(self, x_noisy, t, cond, *args, **kwargs):
        assert isinstance(cond, dict)
        diffusion_model = self.model.diffusion_model
        cond_txt = torch.cat(cond['c_crossattn'], 1)
        cond_prex = torch.cat(cond['c_concat'], 1)
        cond_norm = torch.cat(cond['c_norm'], 1)

        control = self.control_model(x=x_noisy, prex=cond_prex, timesteps=t, context=cond_txt)
    ...
    def log_images(self, batch, N=16, n_row=2, sample=False, ddim_steps=50, ddim_eta=0.0, return_keys=None,
        ...
        if unconditional_guidance_scale > 1.0:
            ...
            uc_norm = c_norm
            uc_full = {"c_concat": [uc_cat], "c_crossattn": [uc_cross], "c_norm": [uc_norm]}
    ...
    def configure_optimizers(self):
        ...
        params += list(self.model.diffusion_model.input_adalns.parameters())
