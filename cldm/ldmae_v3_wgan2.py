...
class AutoEncLDM(LatentDiffusion):
    ...
    def obtain_identity_disc_label(self, batch, realx, fakex, optimizer_idx):
        '''
        realx/fakex: [B * T, C, H, W]
        '''
        B, T = batch[self.frame_index_key].shape
    ...
    def training_step(self, batch, batch_idx, optimizer_idx):
        ...
        if optimizer_idx == 0:
            ...
            samplex_temporal, labels_temporal = self.obtain_time_disc_label(batch, x_recon, real=True)
            ...
            for p in self.spatial_discriminator.parameters():
                p.requires_grad = True
            samplex_spatial, labels_spatial = self.obtain_identity_disc_label(batch, x_start, x_recon, 0)
        ...
        if optimizer_idx == 1: # train discriminator
            ...
            logit_start = self.temporal_discriminator(cat_x_start)
            logit_recon = self.temporal_discriminator(cat_x_recon)
            ...
            # obtain spatial discriminator results
            samplex_spatial, labels_spatial = self.obtain_identity_disc_label(batch, x_start, x_recon, 1)
            logits_spatial = self.spatial_discriminator(samplex_spatial)
