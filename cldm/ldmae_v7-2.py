...
class AutoEncLDM(LatentDiffusion):
    ...
    def obtain_spatial_samplelabels(self, batch, realx, fakex, optimizer_idx):
        """ Ensure indexes of inputs are unsorted!!!
    ...
    def obtain_temporal_samplelabels(self, batch, x, real):
        """
        realx/fakex: (b t) c h w
        return label: 0 - fake, 1 - real
        """
        ...
        if real:
            labels = pre_indexes < cur_indexes
        else:
            labels = torch.zeros_like(pre_indexes)
        ...
        samples = torch.cat([prex, curx], dim=2) # (b t) c 2 h w
        return samples, labels
    ...
    def training_step_wgan(self, batch, batch_idx, optimizer_idx):
        ...
        if optimizer_idx == 0:
            ...
            samplex_temporal, labels_temporal = self.obtain_temporal_samplelabels(batch, x_recon, real=True)
        ...
        if optimizer_idx == 1: # train discriminator
            ...
            # obtain temporal discriminator results
            samplex_start, labels_start = self.obtain_temporal_samplelabels(batch, x_start, real=True)
            samplex_recon, labels_recon = self.obtain_temporal_samplelabels(batch, x_recon, real=False)
            logits_start = self.temporal_discriminator(samplex_start)
            logits_recon = self.temporal_discriminator(samplex_recon)
            d_loss_start = self.adversarial_loss(logits_start, labels_start)
            d_loss_recon = self.adversarial_loss(logits_recon, labels_recon)
            d_loss_temporal = (d_loss_start + d_loss_recon) / 2
            # obtain spatial discriminator results
